...
class VisualizationDemo(object):
    ...
    def run_on_image(self, image, classes_to_keep=None):
        """
        Args:
        ...
        #human_num = 792
        #vase_num = 1138
        #new_predictions = {'instances': []}
        # breakpoint()
        # for i in range(len(predictions['instances'])):
        #     pred_instance = predictions['instances'][i]
        #     if pred_instance.pred_classes.item() == human_num:
        #         new_predictions['instances'].append(pred_instance)
        #     elif pred_instance.pred_classes.item() == vase_num:
        #         new_predictions['instances'].append(pred_instance)
        instances = predictions['instances']
        #instances = instances[(instances.pred_classes ==human_num) + (instances.pred_classes ==vase_num)]
        #only keep the instances inside classes_to_keep
        if not(classes_to_keep is None):
            for i, c in enumerate(classes_to_keep):
                if i ==0:
                    wheres = instances.pred_classes == c 
                else:
                    wheres += instances.pred_classes == c 
            instances = instances[wheres]
